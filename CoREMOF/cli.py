...
def build_parser() -> argparse.ArgumentParser:
    ...
    split_parser.add_argument(
        ...
        help=(
            ...
            "are excluded. rac5 uses all 264 finite binary64 values after -0.0 to "
            ...
            "identity_status/group/size: v26.0.2 preserves audited "
            "v26.0.1 components, then transitively unions exact database-namespaced "
            "source ID, complete MOFid-v2, or complete MOFid-v1 text-equality edges "
            "with no precedence. Each group and identity_size count one transitive "
            "connected component of structures, not edges or identifiers. Missing "
            "identifiers add no edge. It is not proof of "
            ...
            "alter structures or use fuzzy matching. Its preserved v26.0.1 seed uses "
            "the audited v11-only cleanup: cleaned refcodes remove semicolon suffix, "
            "path/CIF suffix, all whitespace, and one terminal ASR_pacman, FSR_pacman, "
            "ION_pacman, ION_ASR_pacman, or ION_FSR_pacman bundle and are "
            "casefolded without NFKC or a database namespace; v11 MOFids remove the "
            "semicolon suffix, repair literal MOFidv2., collapse/casefold without "
            "NFKC, remove terminal v1 .no_ref, and reject leading MOFid NA families. "
            "rac5_topology (RT-) and "
            "mofid_v2_topology (M2T-) require complete exact RAC5/MOFid plus successful "
            ...
            "runtime/path/diagnostics are excluded. M2T remains provisional whenever "
            "its MOFid-v2 input is provisional. structure_matcher_strict (SM-) is a "
            ...
            "matcher, or asymmetric cases are unavailable, not nonmatches. "
            "Reported directional displacement divided by (V/Nsites)^(1/3) is "
            "dimensionless, not angstrom RMSD. SM components are not all-pairs claims; "
            "direct edges are authoritative. RT/M2T/SM prefixes use a criterion-bound "
