...
satur = 4.1 * 1000 # vapor pressure for TIP4P @ 298 K

def get_mol_frac(tot_p, RH, others):

    water_p = satur * RH # water pressure

    others_p = [water_p]
    for i in range(len(others)):
        others_p.append((float(tot_p) - float(water_p)) * others[i])  # calculate pressure 4 other gases
        mol_frac_all = [(1/float(tot_p))*i for i in others_p]
    return mol_frac_all
