...
def main():
    ...
    parser.add_argument('--out_csv', type=str, default="CO2", help='the output CSV file name')
                ...
                for f in os.listdir(path_folder):
                    if f.endswith(".data"):
                        ...
                        if data_found:
                            break
                if not data_found:
                    single_data.extend(np.zeros(2))

                all_data.append(single_data)
                    
            columns = ["Pressure","Uptake","Uptake_error"]
            df_all_data = pd.DataFrame(all_data, columns=columns)
            output_path = s + "_" + args.out_csv + "_" + t + ".csv"
