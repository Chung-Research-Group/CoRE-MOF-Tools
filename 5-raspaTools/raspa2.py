...
class Generate:
    ...
    def Widom(self):
        S, T = load_lists("s_list", "t_list", kwargs=self.kwargs)
        for s in S:
            if self.use_pacman:
                predict_charge(os.path.join(self.kwargs["cif_folder"], s))
                s += "_pacman"
            for t in T:
                folder = self.prepare_files(s, t=t)
                submit_job("submit.sh", folder)

    def EnergyHist(self):
        S, G, T = load_lists("s_list", "g_list", "t_list", kwargs=self.kwargs)
        for s in S:
            if self.use_pacman:
                predict_charge(os.path.join(self.kwargs["cif_folder"], s))
                s += "_pacman"
            for g in G:
                for t in T:
                    folder = self.prepare_files(s, t=t, g=g)
                    submit_job("submit.sh", folder)
...
class Result:
    ...
    def _henry_heat(self):
        S, _, T, _ = self.load_data()
        for s in S:
            for t in T:
                T_data = []
                output = os.path.join(s, str(t), "Output", "System_0")
                if not os.path.isdir(output):
                    continue
                henry_pairs = []
                heat_pairs = []
                ...
                for file in os.listdir(output):
                    if file.endswith(".data"):
                        ...
                        key_word1 = "] Average Henry coefficient:"
                        key_word2 = "Average  <U_gh>_1-<U_h>_0:"
                        data_line1 = [line for line in data_lines if key_word1 in line]
                        data_line2 = [line for line in data_lines if key_word2 in line]
                       
                        for i in range(len(data_line1)):
                            
                            parts = re.split(r'\s+', data_line1[i])
                            
                            henry = float(parts[5])
                            henry_e = float(parts[7])
                            henry_pairs.append((henry, henry_e))

                        for i in range(len(data_line2)):

                            parts = re.split(r'\s+', data_line2[i])

                            heat = float(parts[9]) - 8.314*float(t)/1000
                            heat_e = float(parts[11])
                            heat_pairs.append((heat, heat_e))

                P_data = []
                for i in range(max(len(henry_pairs), len(heat_pairs))):
                    if i < len(henry_pairs):
                        P_data.extend(henry_pairs[i])
                    else:
                        P_data.extend((None, None))
                    if i < len(heat_pairs):
                        P_data.extend(heat_pairs[i])
                    else:
                        P_data.extend((None, None))
                T_data.append(P_data)

                max_n = len(T_data[0]) // 4
                column_names = []
                for i in range(max_n):
                    column_names.extend([f"Henry{i}", f"Henry{i}_e", f"Heat{i}", f"Heat{i}_e"])
                df = pd.DataFrame(T_data, columns=column_names)
                df.to_csv(f"{self.out_folder}/{s}_{t}.csv", index=False)
...
class Check:
    def __init__(self, task, use_pacman, **kwargs):
        ...
        self.kwargs = kwargs
        self.s_list = kwargs.get("s_list")
        self.p_list = kwargs.get("p_list") if task in ["SingleAdsorption", "MixtureAdsorption"] else None
        self.t_list = kwargs.get("t_list")
        self.g_list = kwargs.get("g_list") if task in ["SingleAdsorption", "EnergyHist"] else None
        self.check_status()
